...
We address the task of *Caption-Question Relevance Classification*: given a descriptive caption of a remote-sensing image and a visual question (together with its answer), determine whether the question is semantically related to the caption (binary classification, labels 1/0). This serves as a foundational baseline before moving to Transformer-based architectures.
...
// Comment on which n-gram range performed best and why.
...
// Discuss whether stopword removal or punctuation removal helped or hurt.
...
// Then write 2-3 sentences interpreting the sparse vs dense gap.
// Likely: TF-IDF outperforms because lexical overlap between caption and question
// is a strong signal for this task, and averaging word embeddings dilutes it.

// TODO(manual): Reference the "Sparse vs Dense Feature Comparison" bar chart
// generated by the notebook (Section 8.1). Include it as a figure here if desired.
...
//
// Then comment on what these features reveal about the task.
// E.g., "Features related to spatial descriptions (e.g., 'located', 'central')
// are strong indicators of relevance, while mismatched domain terms indicate
// unrelated pairs."
...
// TODO(manual): For each of the 5 FP examples from the notebook output:
// 1. Briefly describe the caption and question
// 2. Explain WHY the model was confused (e.g., shared vocabulary, similar scene type)
// 3. Categorize the error type (e.g., "Lexical Overlap", "Domain Similarity",
//    "Generic Question", "Shared Object Category")
//
// Example format:
...
// TODO(manual): Same as above for the 5 FN examples.
// Common patterns might include:
// - "Paraphrasing": question uses synonyms not in the caption
// - "Implicit Reference": question asks about a feature described indirectly
// - "Low Overlap": question and caption share no significant n-grams despite being related

= Conclusions

// TODO(manual): After filling in all numeric results above, refine this section.
// The structure below is ready; just verify the claims match the actual numbers.
...
*Future Work*: Contextual embeddings (BERT, RoBERTa) should capture paraphrasing and implicit relevance patterns where bag-of-words models fail. Ensemble approaches combining TF-IDF with dense features could also improve robustness.